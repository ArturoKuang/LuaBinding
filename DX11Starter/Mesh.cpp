...
Mesh::Mesh(Vertex * vertices, int vertexCount, unsigned int * indices, int indexCount, ID3D11Device* device)
{
    ...
    vbd.ByteWidth = sizeof(Vertex) * vertexCount;     // how much space we need = number of vertices in the buffer
...
Mesh::Mesh(char * filename, ID3D11Device* device, XMFLOAT4 color)
{
    // File input object
    ...
    D3D11_BUFFER_DESC vbd;
    vbd.Usage = D3D11_USAGE_IMMUTABLE;
    vbd.ByteWidth = sizeof(Vertex) * vertCounter;     // how much space we need = number of vertices in the buffer
    vbd.BindFlags = D3D11_BIND_VERTEX_BUFFER; // Tells DirectX this is a vertex buffer
    vbd.CPUAccessFlags = 0;
    vbd.MiscFlags = 0;
    vbd.StructureByteStride = 0;
